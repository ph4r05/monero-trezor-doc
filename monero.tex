...
%opening
\title{Monero wallet Trezor integration draft v0.2.5}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item generate transaction master key $k_{mst} = H(TsxData \; || \; r \; || \; c_{tsx})$.
	...
	\begin{enumerate}
		...
		\item Compute HMAC on the data: HMAC($T_{in,i} \; || \; vin_i$), HMAC key: $H(k_{hmac} \; || \; \text{"txin"} \; || \; i)$.
...
36
>>> sys.getsizeof(2**256-1)
60
>>> sys.getsizeof(b'00'*32)
97
\end{lstlisting}

Monero works in EC with 32 B numbers. To store a 32 B number it takes 60 B in integer representation and 97 B in the hex-coded byte string representation (some ed25519 libraries and mininero use this hex-coded representation). For scalars it is apparently more effective to store integers naturaly without hex-coding, saving both memory and CPU cycles with recoding.

EC point arithmetics can use classic point coordinates $(x, y)$ or extended Edwards point coordinates $(x,y,z,t)$. It takes 64 and 80 B to store tuple of 2 and 4 elements respectivelly. It thus take 184 B and 320 B to store an EC point in the natural form compared to the 97 B byte representation.
