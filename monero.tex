...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Generate ENC key $k_{enc} = H^2\left(\text{"enc"} \; || \; k_{mst}\right)$
...
The AES-GCM is used with the key $H^2\left(k_{enc} \; || \; \text{"txin-alpha"} \; || \; i\right)$. The $\alpha_i$ is then passed back to the Trezor in the signing phase. Alternative to offloading is deterministic mask generation described in section \ref{mask_reconstruction}.
