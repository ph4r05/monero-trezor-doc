...
\begin{enumerate}
	...
	\item $H \rightarrow T$: Send all transaction outputs $T_{out}$, one by one. $T_{out,i} = ((A_d, B_d), \text{amount})_i$. Change output $T_{chx}$ is the last transaction output. 
	\begin{enumerate}			
		\item Generate the key derivation $\mathcal{D}_{out}$, i.e. the \emph{amount key}.
        \item Store the \emph{amount key} in the Trezor transaction state.
        ...
        \item HMAC of the output transaction data HMAC($T_{out,i} \; || \; P_{out} \; || \; P_{out,j} \; || \; tx.vout_i \; || \; tx.extra_i $) with HMAC key: $H(k_{hmac} \; || \; \text{"txout"} \; || \; i)$.
        
        %\item Compute range proof on the 
        
		\item Return the transaction prefix fields $tx.vout_i$ and $tx.extra_i$.
