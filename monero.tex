...
\subsubsection{Batched Bulletproof} 
In the new Monero fork the batched Bulletproofs are implemented. Batched Bulletproofs are ZK proofs about multiple statements in one proof. The size increase is logarithmic in the number of statements. Statement is a range proof about the transaction output value. By batching one proof can cover the whole transaction.

For example BP for one transaction output L, R fields have 6 scalars. BP for two transaction outputs has 7 scalars in L, R fields.
