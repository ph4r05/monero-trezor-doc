...
All signatures are performed in the Trezor on the set of spending TXO in one step. Output rangeproofs are offloaded to the host as explained below.
...
The first part of the \verb|proveRange()| function generates random $a_i$ values and $C_i$ which are further used to generate rangeproof signatures. These values will be generated in Trezor to have random values under our control as $a_i$ are used as a mask later. We need to protect the amount commitment and the mask from tampering.
