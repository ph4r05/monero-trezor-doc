...
\usepackage{tikz}  % has to be loaded after xcolor and todonotes
\usetikzlibrary{calc}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item Compute a derivation $\mathcal{D}_{in,i} = 8aR_{in}$
...
The more detailed transaction construction can be found in our full paper \cite{full_paper} (complete equations, handles subaddresses).
...
\paragraph{Secure multi-party implementation} In the \cite{full_paper} we designed MPC version of the Bulletproof which computes the Bulletproof with help of the Host without leaking input masks to the host. The MPC version runs in constant memory, thus we can compute BP of arbitrary size.
...
\begin{itemize}
	...
	\item \citetitle{cryptoeprint:2017:1066} \url{https://eprint.iacr.org/2017/1066.pdf}
	\item Full paper on the current Bulletproof \cite{full_paper} implementation and its MPC version.
\end{itemize}
\section{Transaction signing protocol}

The description of the subdivided transaction signing protocol follows. The reference implementation is: \url{https://github.com/trezor/trezor-firmware/tree/master/core/src/apps/monero/signing}. See also \cite{full_paper} for more detailed protocol description.
...
\begin{enumerate}
	\item Initialize the new protocol run as in the original one-step protocol. Send TsxData(version, payment\_id, unlock\_time, outputs, change\_dest, num\_inputs, mixin, fee, account\_id, rsig\_data with rsig\_type and rsig\_grouping, integrated\_indices, hard\_fork, monero\_client\_version).
	\begin{enumerate}
	...
	$T_{in,i} = ((\text{idx}, P_j,C_j)_j, \text{amount}, \text{mask}, \pi, \text{sub\_addr\_minor}, \text{pubs}, \text{additional\_keys} \dots)_i$.
	\\\\%
	In the newest client protocol versions sub address index is sent together with the UTXO thus we can recompute the keys without need to precompute subaddresses as Monero wallet does. 
	\begin{enumerate}
	...
	\item $H \rightarrow T$: Host sorts input according to key images in descending order.\\\\
	%
	Send all transaction inputs $T_{in}$, one by one, as \verb|tx_source_entry|, with corresponding $tx.vin_i$ part and HMAC and original sorting index (prior key image sort). Trezor verifies the HMAC (using original sort index as it enters HMAC key derivation), checks descending ordering on the key images and incrementally hashes $tx.vin_i$ to the \emph{transaction\_prefix\_hash}. Trezor also hashes $\text{pseudo\_out}_{in}$ to the RctSigBase.
	
	\item $H \rightarrow T$: All inputs set: Finalizes phase of setting the inputs.
	...
	%
	If range proof is being offloaded, after all outputs from the output batch has been sent, another extra message of this type is sent from the host (is\_offloaded\_bp=True) which contains the generated range proof. As the output masks are deterministic and generated after the output has been processed, one extra message is needed.
	\begin{enumerate}			
		...
		\item Return the transaction prefix fields $tx.vout_i$ and $tx.extra_i$, $rsig_i$, $\text{out\_pk}_i$, $\text{ecdh\_info}_i$ and corresponding HMACs. Return also $a_i$ commitment mask if range proof is offloaded.
	...
	\item $H \rightarrow T$: RingCT Sign $T_{in}$. Send all transaction inputs $T_{in}$, one by one, with hmacs, pseudo output, alpha (encrypted), spending key (encrypted), original sorting index (prior key image sort).
	\begin{enumerate}
		\item Check the HMAC, Key image ordering.
		
		\item If the input is the last, recompute pseudo outputs so commitment masks are equal $\sum \alpha = \sum a_{i}$.
		
		\item Compute RingCT MG signature $S_i$ on the input and the \emph{final\_message}.
		
		\item In the client version 3+, the MLSAG / CLSAG signature is returned encrypted with Chacha20Poly1305 under key derived from the opening key. The opening key is returned in the following step if protocol finishes successfully.
		
		\item Return signature and recomputed pseudo output. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.\textwidth,,trim={0 24cm 0 1cm},clip, angle=0, scale=0.7]{img/tsx_state.pdf}
	\caption{Simplified transaction state for incremental signing}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=1.\textwidth,trim={0 23cm 0 1cm},clip, angle=0, scale=0.7]{img/tsx_state_detail.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth,,trim={0 5cm 0 1cm},clip, angle=0]{img/subdivided_rsig.pdf}
...
% http://ftp.cvut.cz/tex-archive/graphics/pgf/contrib/pgf-umlsd/pgf-umlsd-manual.pdf
\begin{figure}[H]
\centering
...
{
  // blockchain index of the UTXO, key, Commitment
  typedef std::pair<uint64_t, rct::ctkey> output_entry;

  // index + key + optional ringct commitment
  std::vector<output_entry> outputs;  
  
  // index in outputs vector of real output_entry (\pi)
  size_t real_output;                 
  ...
  uint64_t amount;                    //received amount
  bool rct;                           //true if the output is rct
  rct::key mask;                      //range proof amount mask
