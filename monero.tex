...
\begin{itemize}
	...
	\item Private spending key: $x = H_s(aR) + b + H_s(a \; || \; i \; || \; j)$
...
\begin{enumerate}
	...
	\begin{enumerate}
        ...
        \item Compute the transaction prefix \emph{vout} and \emph{extra}.
		...
		\item Return the transaction prefix \emph{vout} and \emph{extra}.
	...
	\begin{enumerate}
		...
		\item Compute the Range proof in the host.
		
		\item Compute \emph{mask}, \emph{output commitment}, \emph{ecdhInfo}.
		
		\item Compute the HMAC(\emph{asig} || \emph{Ci}) with the HMAC key $H(k_{hmac} \; || \; \text{"txoutres"} \; || \; i)$. Store the HMAC to the transaction state.
