...
%opening
\title{Monero wallet Trezor integration draft v0.3.2}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth,,trim={0 3cm 0 1cm},clip, angle=0]{img/subdivided_seq.pdf}
...
\begin{enumerate}
	...
	\item Send all transaction inputs $T_{in}$, one by one, with hmacs, pseudo output, alpha.
	\begin{enumerate}
		\item Check the HMAC
...
The naive incremental version computes range proof bit by bit, splitting it to 64 round-trips with minimal memory overhead per round-trip.
The Range proof generator algorithm cannot be trivially transformed to an incremental version as it requires 2 dependent passes over the 64 bits representing the amount. The first pass affects values: $a_i, \alpha_i, C_i, asig.ee, asig.s_1$. The second pass uses $a_i, \alpha_i, C_i, asig.ee, asig.s_1$ values to compute final $asig.s_0, asig.s_1$ values. The problem is that $asig.ee$ is affected by each input value from the first pass and used in each step of the second pass. The incremental version thus would have to generate 128 random scalar masks $a_i, \alpha_i$ in advance as the exact same values are needed in the second pass. This leads to significant memory performance degradation, $4096$~B overhead.

To overcome this issue we have to modify scalar mask generation method. Instead of generating the masks using the system random number generator we use own seeded random number generator as shown in Figure \ref{eq:masks}.

\begin{figure}[H]

	\begin{equation}
	\begin{split}
	a_i &= H^4(k_{hmac} \; || \; idx \; || \; i \; || \; "rsig-rnd-ai" \; ) \; \text{mod} \; l  \\
	\alpha_i &= H^4(k_{hmac} \; || \; idx \; || \; i \; || \; "rsig-rnd-alphai" \; ) \; \text{mod} \; l
	\end{split}
	\end{equation}

	\caption{$a_i, \alpha_i$ Generation. $idx$ is the output index in the transaction, $l$ is Ed25519 generator order.} \label{eq:masks}
\end{figure}

This modification assumes random oracle model. The output of a Keccak-256 is assumed to be indistinguishable from random number stream. The $H$ building block can be simply replaced by AES-CTR which is also commonly used as a standard PRNG. 

With this modification the algorithm can be transformed to incremental version with computing range proof bit by bit. The second pass re-computes required $a_i, \alpha_i$ values.
