...
\subsubsection{Bulletproof} The offloading mechanism is very similar to the Borromean range proof. The random mask and the amount commitments \verb|V| are computed in the Trezor to make sure it was not tampered with (simple computation step with amount and mask). 
...
In the new Monero fork the batched Bulletproofs are implemented. Batched Bulletproofs are ZK proofs about multiple statements in a single proof. The proof size increase is logarithmic in the number of statements and statement sizes. Statement is a range proof about the transaction output value. Batching can cover the whole transaction with one BP.

For example BP for one transaction output L, R fields have 6 scalars. BP for two transaction outputs has 7 scalars in L, R fields so space saved is significant. Verification times are also better for batched Bulletproofs.
...
\begin{itemize}
	...
	\item {\it RangeProofMultiOutputBulletproof} BP batching $2^x$ outputs up to the upper bound $16$. Transaction can have multiple BP of this type so probably intended to use with many transaction outputs.
...
\begin{itemize}
	\item Scalars \verb|A, S, T1, T2, taux, mu, a, b, t|
	\item Vector of commitments \verb|V| of size $M$
	\item Vector of scalars \verb|L, R| of size $log(MN)$
\end{itemize}
...
In order to hide a commitment mask \verb|gamma| from the host the BP generation would have to be slightly changed. Trezor sends only a commitments \verb|V| and portion of \verb|taux| value. 
...
\begin{algorithm}[H]
	\caption{Pseudocode of the BP part - mask hiding. $\gamma$ is used only in the formulas in the algorithm. Value $x$ is result of a complex computation from the amount and constant values. } \label{alg:bp_mask_hiding}
	\begin{algorithmic}[1]
		\Function{Bulletproof}{$amounts, \gamma$}
		...
		\State $\alpha \gets \text{detScalar}()$
		...
		\State $z \gets H_s(y)$		
		\State $\tau_1 \gets \text{detScalar}()$
		\State $\tau_2 \gets \text{detScalar}()$
		
		\State $x \gets \text{bp\_func}(z, \tau_1, \tau_2, t_1, t_2)$ \Comment{Complex computation}
		\State $\tau_x \gets \tau_1 x + \tau_2 x^2 + \sum_{i=0}^{M} \gamma_i z^{i+2}$
		\State $\mu \gets \rho x + \alpha$
		\Comment{BP: $(V, A, S, T1, T2, \tau_x, \mu, L, R, a, b, t)$}
		\EndFunction	
	\end{algorithmic}
\end{algorithm}


From algorithm \ref{alg:bp_mask_hiding}: In 
order to protect $\gamma$ from the attacker the Trezor has to compute commitment vector \verb|V| and part of the $\tau_x$, the sum $\sum_{i=0}^{M} \gamma_i z^{i+2}$. For that Trezor needs to evaluate BP generator up to the $z$ which is computable with constant memory as we use optimizations described in the following paragraph.
This yields to single round trip protocol, Trezor only needs to send the values before the BP generation to the host.

In order to check also $tau_{1,2}$ we would have to split BP generation to two round trips. Prover needs to send $x$ to the Trezor so it can compute $\tau_x, \mu$ effectively. This is quite a complication and left just as a suggestion.
...
By using these tricks it is possible to compute even large BPs on the Trezor but the computation time would be rather high. In the current setup BP with one output takes 13 seconds, with two outputs 22 seconds to generate and uses 20 kB and 40 kB of RAM respectivelly.
