...
If the transaction has 30 TXOs with mixin set to 20 the overall memory requirements for inputs are 41~280~B, output is 39~360~B. This is the minimal memory needed to do the computation. Signature itself consumes roughly aroung 128~kB of memory per TXO.

\subsection{Signature details}
...
Thus if one can build transaction prefix hash securely in the Trezor we can use the protocol subdivision on the transaction inputs, ringCT signing one input TXO at a time.
