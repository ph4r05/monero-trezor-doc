...
As seen from the algorithm \ref{alg:bp_mask} it is not triviall to hide a commitment mask \verb|gamma| from the host in the offloaded BP generation. Trezor could generate the commitment vector \verb|V| which hides the mask \verb|gamma| but the mask is also used for \verb|taux| computation. When using deterministic masks the attacked knows other scalar values used for \verb|taux| computation so even if \verb|taux| is computed by the Trezor attacker could extract the \verb|\gamma| easily (simple modular arithmetic). 
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State $t_1, t_2 \gets \text{bp\_func}(y, z, aL, aR, sL, sR)$ \Comment{Complex computation}
