...
\begin{enumerate}
	...
	\item Trezor.io Monero RPC node. Full-node local vs. remote node (trusted). A user can run his local node. The node is set in the trezor.io menu. By default, there can be implicit node owned by the SatoshiLabs / Monero open nodes.
...
\noindent TBD. Depends on the final requirements.
